...
use leptos::*;
use lucide_leptos::{
    Calendar, Globe, Heart, Loader, Search, Trophy, Flame, Target, Award, TrendingUp, Users,
    User as UserIcon,
};
...
pub fn UserDemo(
    ...
    // State
    let (query_by_id, set_query_by_id) = signal("".to_string());
    let (query_by_username, set_query_by_username) = signal("".to_string());
    let user_data: RwSignal<Option<User>> = RwSignal::new(None);
    let selected_mode: RwSignal<Mode> = RwSignal::new(Mode::Osu);

    let handle_query_by_id_input = move |ev| {
        let input_element = event_target::<HtmlInputElement>(&ev);
        set_query_by_id.set(input_element.value());
    };

    let handle_query_by_username_input = move |ev| {
        let input_element = event_target::<HtmlInputElement>(&ev);
        set_query_by_username.set(input_element.value());
    };

    let handle_search = move |_| {
        let query_id = query_by_id.get();
        let query_username = query_by_username.get();

        // Check that exactly one field is filled
        if query_id.is_empty() && query_username.is_empty() {
            set_result.set("Please enter either a user ID or username to search".to_string());
            return;
        }

        if !query_id.is_empty() && !query_username.is_empty() {
            set_result.set("Please enter only one search criteria (ID or username)".to_string());
            return;
        }

        set_is_loading.set(true);
        set_result.set("Searching...".to_string());
        ...
        spawn_local(async move {
            ...
            let result = if !query_id.is_empty() {
                // Search by ID
                let user_id: u32 = match query_id.parse() {
                    Ok(id) => id,
                    Err(_) => {
                        set_result.set(format!("Please enter a valid user ID (numeric)"));
                        set_is_loading.set(false);
                        return;
                    }
                };
                client.users.get_user(user_id, Some(mode), None).await
            } else {
                // Search by username
                client.users.get_user_by_username(&query_username, Some(mode), None).await
            };
        ...
        <div class="space-y-6">
            ...
            <div class="p-6 rounded-lg border bg-white dark:bg-slate-800/50 border-gray-200 dark:border-slate-700/50">
                ...
                <div class="space-y-4">
                    <p class="text-sm text-gray-600 dark:text-slate-400">
                        "Enter a user ID or username to fetch their osu! profile information."
                    </p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                value=move || query_by_id.get()
                                on:input=handle_query_by_id_input
                                disabled=move || !query_by_username.get().is_empty()
                                class="py-2.5 px-4 w-full rounded-lg border transition-all focus:border-pink-500 dark:focus:border-cyan-500 focus:outline-none bg-white dark:bg-slate-900 border-gray-300 dark:border-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-500 dark:placeholder-slate-500 disabled:bg-gray-100 dark:disabled:bg-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value=move || query_by_username.get()
                                on:input=handle_query_by_username_input
                                disabled=move || !query_by_id.get().is_empty()
                                class="py-2.5 px-4 w-full rounded-lg border transition-all focus:border-pink-500 dark:focus:border-cyan-500 focus:outline-none bg-white dark:bg-slate-900 border-gray-300 dark:border-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-500 dark:placeholder-slate-500 disabled:bg-gray-100 dark:disabled:bg-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        disabled=move || {
                            let id = query_by_id.get();
                            let username = query_by_username.get();
                            is_loading.get() || (id.is_empty() && username.is_empty()) || (!id.is_empty() && !username.is_empty())
                        }
            ...
            <div class="p-6 rounded-lg border bg-white dark:bg-slate-800/50 border-gray-200 dark:border-slate-700/50">
                ...
                <ul class="space-y-2 text-xs text-gray-600 dark:text-slate-400">
                    ...
                    <li>"• Search by either numeric user ID or username (choose one)"</li>
