...
pub fn UserBestDemo(
    ...
    let no_best_found = Memo::new(move |_| tr!("api-demo-no-best-found"));
    let error_prefix = Memo::new(move |_| tr!("api-demo-error", {"error" => ""}));

    // Best score result templates
    let best_play_tpl = Memo::new(move |_| tr!("api-demo-best-play", {"index" => "{$index}"}));
    ...
    // API Request: User Best
    let fetch_user_best = move |_| {
        ...
        let no_best_found_str = no_best_found.get();
        let best_play_template = best_play_tpl.get();
        ...
        spawn_local(async move {
            ...
            let params = GetUserBestParams::default()
                .user(user)
                .mode(selected_mode)
                .limit(selected_limit);

            match client.user.get_user_best(params).await {
                Ok(bests) => {
                    // Set raw JSON
                    set_raw_json.set(format!("{:#?}", bests));

                    if bests.is_empty() {
                        set_result.set(no_best_found_str);
                    } else {
                        let mut result_str = String::new();

                        for (idx, best) in bests.iter().enumerate() {
                            if idx > 0 {
                                result_str.push_str("\n---\n");
                            }
                            ...
                            // Play number
                            result_str.push_str(
                                &best_play_template.replace("{$index}", &(idx + 1).to_string()),
                            );
                            result_str.push_str("\n");

                            // Beatmap ID
                            result_str.push_str(
                                &beatmap_id_template.replace("{$id}", &best.beatmap_id),
                            );
                            result_str.push_str("\n");

                            // Score
                            result_str.push_str(&score_template.replace("{$score}", &best.score));
                            result_str.push_str("\n");

                            // Combo
                            result_str.push_str(&combo_template.replace("{$combo}", &best.maxcombo));
                            result_str.push_str("\n");

                            // Accuracy hits
                            let hits = accuracy_hits_template
                                .replace("{$c300}", &best.count300)
                                .replace("{$c100}", &best.count100)
                                .replace("{$c50}", &best.count50)
                                .replace("{$miss}", &best.countmiss);
                            result_str.push_str(&hits);
                            result_str.push_str("\n");

                            // Rank
                            result_str.push_str(&rank_template.replace("{$rank}", &best.rank));
                            result_str.push_str("\n");

                            // Mods
                            result_str.push_str(&mods_template.replace("{$mods}", &best.enabled_mods));
                            result_str.push_str("\n");

                            // PP
                            result_str.push_str(&pp_template.replace("{$pp}", &best.pp));
                            result_str.push_str("\n");

                            // Date
                            result_str.push_str(&date_template.replace("{$date}", &best.date));
                            result_str.push_str("\n");

                            // Perfect combo
                            let perfect_status = if best.perfect == "1" { "Yes" } else { "No" };
